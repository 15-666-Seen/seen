...
//"WalkPoint" represents location on the WalkMesh as barycentric coordinates on
//a triangle:
struct WalkPoint {
  ...
  // NOTE: by convention, if WalkPoint is on an edge, indices/weights will be
  // arranged so that weights.z will be 0.0.
  WalkPoint(glm::uvec3 const &indices_, glm::vec3 const &weights_)
      : indices(indices_), weights(weights_) {}
  WalkPoint() = default;
};
...
struct WalkMesh {
  ...
  // triangle (a,b,c), and is useful for checking what's over an edge from a
  // given point:
  ...
  void
  walk_in_triangle(WalkPoint const &start, //[in] starting location on triangle
                   glm::vec3 const &step,  //[in] step to take (in world space).
                                           //Will be projected to triangle.
                   WalkPoint *end,         //[out] final position in triangle
                   float *time //[out] time at which edge is encountered, or 1.0
                               //if whole step is within triangle
  ) const;
  ...
  //    - *rotation brings vectors in the plane of start.triangle.xyz to vectors
  //    in the plane of end->triangle.xyz
  //    - function returns true
  bool cross_edge(WalkPoint const &start, //[in] walkpoint on triangle edge
  ...
  glm::vec3 to_world_smooth_normal(WalkPoint const &wp) const {
    return glm::normalize(wp.weights.x * normals[wp.indices.x] +
