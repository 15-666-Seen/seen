...
#ifdef _WIN32
  // when compiled on windows, unhandled exceptions don't have their message
  // printed, which can make debugging simple issues difficult.
  try {
#endif
    ...
    SDL_Window *window = SDL_CreateWindow(
        "scene viewer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800,
        800,
        SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE // uncomment to allow resizing
            | SDL_WINDOW_ALLOW_HIGHDPI // uncomment for full resolution on
                                       // high-DPI screens
    );
    ...
    if (meshes_file != "") {
      try {
        ...
        std::cerr << "ERROR loading mesh buffer '" << meshes_file
                  << "': " << e.what() << std::endl;
        usage = true;
        buffer = nullptr;
      }
    ...
    if (scene_file != "") {
      try {
        scene = new Scene();
        scene->load(scene_file, [&buffer, &buffer_vao](
        ...
        std::cerr << "ERROR loading scene '" << scene_file << "': " << e.what()
                  << std::endl;
        usage = true;
        scene = nullptr;
      }
    ...
    if (usage) {
      std::cerr << "Usage:\n\t" << argv[0]
                << " <path/to/scene.scene> [path/to/meshes.pnct]" << std::endl;
      return 1;
    }
    ...
    if (meshes_file != "") {
      ...
      std::cout << " no meshes -- consider passing a '.pnct' file as the "
                   "second argument."
                << std::endl;
    }
