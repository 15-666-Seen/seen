...
MeshBuffer::MeshBuffer(std::string const &filename) {
  ...
  if (filename.size() >= 5 && filename.substr(filename.size() - 5) == ".pnct") {
    ...
    Position = Attrib(3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
  ...
  { // read index chunk, add to meshes:
    ...
    for (auto const &entry : index) {
      ...
      if (!inserted) {
        std::cerr << "WARNING: mesh name '" + name + "' in filename '" +
                         filename + "' collides with existing mesh."
                  << std::endl;
      }
    }
  }
  ...
  if (file.peek() != EOF) {
    std::cerr << "WARNING: trailing data in mesh file '" << filename << "'"
              << std::endl;
  }
  ...
  std::cout << " and"; std::cout << " '" << m.first << "'"; if (&m.second !=
  &meshes.rbegin()->second) std::cout << ",";
  }
  std::cout << std::endl;
  */
}
...
const Mesh &MeshBuffer::lookup(std::string const &name) const {
  ...
  if (f == meshes.end()) {
    throw std::runtime_error("Looking up mesh '" + name +
                             "' that doesn't exist.");
  }
  return f->second;
}
...
GLuint MeshBuffer::make_vao_for_program(GLuint program) const {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, buffer);
  auto bind_attribute = [&](char const *name,
                            MeshBuffer::Attrib const &attrib) {
    if (attrib.size == 0)
      return; // don't bind empty attribs
    GLint location = glGetAttribLocation(program, name);
    if (location == -1)
      return; // can't bind missing attribs
    glVertexAttribPointer(location, attrib.size, attrib.type, attrib.normalized,
                          attrib.stride, (GLbyte *)0 + attrib.offset);
  ...
  assert(active >= 0 &&
         "Doesn't makes sense to have negative active attributes.");
  for (GLuint i = 0; i < GLuint(active); ++i) {
    ...
    if (!bound.count(GLuint(location))) {
      throw std::runtime_error("ERROR: active attribute '" + std::string(name) +
                               "' in program is not bound.");
    }
  }
