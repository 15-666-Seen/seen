...
struct MeshBuffer {
  ...
  // These 'Attrib' structures describe the location of various attributes
  // within the buffer (in exactly format wanted by glVertexAttribPointer). They
  // are set when the file is loaded and are used by the "make_vao_for_program"
  // call:
  struct Attrib {
    GLint size = 0;
    GLenum type = 0;
    ...
    Attrib() = default;
    Attrib(GLint size_, GLenum type_, GLboolean normalized_, GLsizei stride_,
           GLsizei offset_)
        : size(size_), type(type_), normalized(normalized_), stride(stride_),
          offset(offset_) {}
  };
