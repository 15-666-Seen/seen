...
// Adapted from https://github.com/cmu15466-gsilvera/15-466-f22-base4/blob/main/Text.hpp
    ...
    for (unsigned int i = 0; i < static_cast<unsigned int>(amnt * num_chars); i++) {
        ...
        if (char_x >= right_bound || text_content[i] == '\n') {
                    ...
                    //newline by word
                        ...
                        if ((char_x + ((ch.Advance >> 6) * ss_scale * (j - i + 10))) >= right_bound) {
                            char_x = pos.x;    
                            char_y -= (y_size + 5.0f);    
            			}	    
