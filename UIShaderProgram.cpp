...
SpriteProgram::SpriteProgram() {
  ...
  unsigned char *data = loadImg(data_path("textures/UI"), "dialogue.png", width,
                                height, channels);
...
static Load<void> setup_buffers(LoadTagDefault, []() {
  ...
  { // vertex array mapping buffer for sprite_program:
    // ask OpenGL to fill vertex_buffer_for_sprite_program with the name of an
    // unused vertex array object:
    ...
    //[Note that it is okay to bind a vec3 input to a vec4 attribute -- the w
    //component will be filled with 1.0 automatically]

    glVertexAttribPointer(sprite_program->UV_vec2,  // attribute
                          2,                        // size
                          GL_FLOAT,                 // type
                          GL_TRUE,                  // normalized
                          sizeof(UIShader::Vertex), // stride
...
UIShader::UIShader(glm::mat4 const &world_to_clip_)
    : world_to_clip(world_to_clip_) {}
...
void UIShader::draw_dialogue_box(glm::mat4x3 const &mat) {
  ...
  // draw(mat * glm::vec4(-1.0f, -1.0f, -1.0f, 1.0f), glm::vec2(1.f, 0.f)); //
  // top
  draw(mat * glm::vec4(-1.0f, 1.0f, -1.0f, 1.0f),
       glm::vec2(0.f, 1.f)); // top left
  draw(mat * glm::vec4(1.0f, -1.0f, -1.0f, 1.0f),
       glm::vec2(1.f, 0.f)); // btm right
  draw(mat * glm::vec4(1.0f, 1.0f, -1.0f, 1.0f),
       glm::vec2(1.f, 1.f)); // top right

  draw(mat * glm::vec4(-1.0f, -1.0f, -1.0f, 1.0f),
       glm::vec2(0.f, 0.f)); // bt, left
  draw(mat * glm::vec4(1.0f, -1.0f, -1.0f, 1.0f),
       glm::vec2(1.f, 0.f)); // btm right
  draw(mat * glm::vec4(-1.0f, 1.0f, -1.0f, 1.0f),
       glm::vec2(0.f, 1.f)); // top left
...
UIShader::~UIShader() {
  ...
  glBufferData(GL_ARRAY_BUFFER, attribs.size() * sizeof(attribs[0]),
               attribs.data(), GL_STREAM_DRAW); // upload attribs array
  glBindBuffer(GL_ARRAY_BUFFER, 0);
  ...
  glUniformMatrix4fv(sprite_program->OBJECT_TO_CLIP_mat4, 1, GL_FALSE,
